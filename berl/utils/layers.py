...
# Virtual batch norm: taken and adapted from https://github.com/torchgan/torchgan/blob/master/torchgan/layers/virtualbatchnorm.py
...
class VirtualBatchNorm(nn.Module):
    ...
    def __init__(self, in_features, scale=True, center=True, eps=1e-8):
        super(VirtualBatchNorm, self).__init__()
        self.in_features = in_features
        self.gamma = nn.Parameter(torch.ones(in_features)) if scale else None
        self.beta = nn.Parameter(torch.zeros(in_features)) if center else None
        self.ref_mu = None
        self.ref_var = None
        self.eps = eps
