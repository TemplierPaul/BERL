...
class CustomES(ES):
    ...
    def set_weights(self):
        ...
        if weight_decay=="linear":
            ...
            self.w = np.array([max(0, np.log(mu + 0.5) - np.log(i)) for i in range(1, n + 1)])  
        else:
            raise NotImplementedError(f"{weight_decay} is not a valid weight decay")
        ...
        if self.config["custom_wi_sum"]==0:
            ...
            assert self.config["custom_wi_sum"]== 1, f"Weights should be summed to 0 or 1, not {self.config['custom_wi_sum']}"
    ...
    def get_weights(self):
        ...
        if sum(self.w) <= 1e-5: 
            ...
            self.w = mask / sum(mask) if max(self.w) <= 1e-5 else self.w / max(abs(self.w))
            return self.w
